...
class SVRCalibration(Calibration):
    def __init__(self, model_save_path: str = ""):
        ...
        if self.svr_y_path.exists() and self.svr_x_path.exists():
            # Use as_posix() — OpenCV FileStorage requires forward-slash paths on Windows
            self.svr_x = cv.ml.SVM.load(self.svr_x_path.as_posix())
            self.svr_y = cv.ml.SVM.load(self.svr_y_path.as_posix())
    ...
    def save_model(self) -> bool:
        ...
        if self.svr_x.isTrained() and self.svr_y.isTrained():
            # Use as_posix() — OpenCV FileStorage requires forward-slash paths on Windows
            self.svr_x.save(self.svr_x_path.as_posix())
            self.svr_y.save(self.svr_y_path.as_posix())
